...
prepareCode = "
(def exec-cmd (lambda (cmd) (begin (def cmd (pull (def args (split (trim cmd) ' ')))) (def args (join args)) (def comp (dot (get_shell) 'host_computer')) (def file (if (dot comp 'File' (get_abs_path cmd)) (dot comp 'File' (get_abs_path cmd)) (if (dot comp 'File' (+ '/bin/' cmd)) (dot comp 'File' (+ '/bin/' cmd)) (if (dot comp 'File' (+ '/usr/bin/' cmd)) (dot comp 'File' (+ '/usr/bin/' cmd)) null)))) (if file (if (| (dot file 'is_folder') (dot file 'is_binary')) (dot (get_shell) 'launch' (dot file 'path') args) (if (dot file 'has_permission' 'r') (begin (def params (split args ' ')) (exec (dot file 'get_content'))) (print 'Permission denied.'))) (print (+ cmd ': command not found'))))))
