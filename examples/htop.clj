(def htop (lambda (target-comp switch-comp) ;pass two computer object, switch-comp is for killswitch, when switch-comp is not running Notepad the loop stops.
    (def bar (lambda (n) (join (list '<color=#21bcff>' (* '#' (ceil (/ n 4))) '<color=#032e41>' (* '-' (- 25 (ceil (/ n 4)))) '</color>') '')))
    (while (indexOf (dot switch-comp 'show_procs') 'Notepad') (begin
        (def tasks (list)) (def cpu 0) (def mem 0)
        ...
        (while lines (begin
            ...
            (push tasks (join (list (if (== (at line 0) 'root') '<color=#ff4b4b>' '<color=#fbfbfb>') (at line 0) ' <color=#20ff98>' (at line 1) ' <color=#21bcff>' (at line 2) ' <color=#21bcff>' (at line 3) ' <color=#baff50>' (at line 4) '</color>') ''))))
        (wait (/ (! (print (+ (join (list '<color=#9d9d9d>Check <color=#20ff98>' (len tasks) ' <color=#9d9d9d>processes on <color=#fbfbfb>' (dot target-comp 'get_name') '<color=#9d9d9d>@<color=#fbfbfb>' (dot target-comp 'public_ip') '<color=#9d9d9d>:<color=#fbfbfb>' (dot target-comp 'local_ip') '\n<color=#fbfbfb>cpu_usage: [' (bar cpu) '<color=#fbfbfb>]==[ <color=#21bcff>' cpu '% <color=#fbfbfb>]\n<color=#fbfbfb>mem_usage: [' (bar mem) '<color=#fbfbfb>]==[ <color=#21bcff>' mem '% <color=#fbfbfb>]</color>\n') '') (format_columns (join (+ (list '<color=#9d9d9d>USER <color=#9d9d9d>PID <color=#9d9d9d>CPU <color=#9d9d9d>MEM <color=#9d9d9d>COMMAND</color>') tasks) '\\n'))) 1)) 2))))))
